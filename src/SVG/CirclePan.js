...
export default function CirclePan(props) {
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative' }}>
      {props.data.map((it, index) => {
        ...
        return (
          <style key={keys[index]}>
            ...
            @keyframes
            {` jin-pan-${index}{from{stroke-dasharray:0,${circum[index]}} to{stroke-dasharray:${
              (circum[index] * it.percent) / 100
            },${circum[index]}}}`}
          </style>
        );
      })}
      ...
      >
        <g fill="none">
          <g className="g-origin-jin" style={{}}>
            {props.data &&
              props.data.map((it, index) => {
                return (
                  <circle
                    ...
                    r={radius[index]}
                    strokeWidth="5"
                    stroke={it.backGround || 'transparent'}
                  />
                );
              })}
            ;
                ...
                return (
                  <g key={index}>
                    ...
                    <text fontSize="10" fill={it.color || color[index]}>
                      <tspan x={120 + (180 * it.percent) / 100 + 22} y={yArray[index] * 1.1}>
                        {`${it.percent}%`}
                      </tspan>
                    </text>
                  </g>
          ...
          <g transform="translate(270,60)">
            {props.data &&
              props.data.map((it, index) => {
                return (
                  <g key={Math.random()}>
                    <circle cx="0" cy={yArray2[index]} r="3" fill={it.color || color[index]} />
                    <text
                      x="14"
                      y={yArray2[index] + 3}
                      fontSize="9px"
                      fill={it.textColor || 'black'}
                    >
                      {it.legend}
                    </text>
                  </g>
